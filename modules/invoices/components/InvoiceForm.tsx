...
const emptyItem = (): InvoiceItem => ({ d: '', components: [], q: 1, r: 0, t: 0 })

interface InvoiceFormProps {
  editId?: string
}

export function InvoiceForm({ editId }: InvoiceFormProps) {
  const router = useRouter()
  const { company } = useAppContext()

  const { data: existing } = useApi<Invoice>(editId ? `/api/invoices/${editId}` : null)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId) {
        await apiMutate(`/api/invoices/${editId}`, 'PUT', payload)
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 space-y-4">
                      ...
                      <div className="grid grid-cols-3 gap-4 pl-8">
                        ...
                        <div>
                          <Label className="text-xs">Rate ({cur})</Label>
                    ...
                    <Button variant="ghost" size="icon" className="text-red-500" onClick={() => removeItem(i)} disabled={items.length <= 1}>
              ...
              <div className="space-y-1 text-right pt-4 border-t">
                <div className="font-semibold text-slate-500">Subtotal: {cur} {total.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
                <div className="flex items-center justify-end gap-3">
                  <Label className="text-xs text-red-500">Discount ({cur})</Label>
                  <Input type="number" value={discount} onChange={e => setDiscount(Number(e.target.value))} className="w-32 text-right border-red-200" />
                </div>
                <div className="flex items-center justify-end gap-3">
                  <Label className="text-xs">Tax ({cur})</Label>
                  <Input type="number" value={tax} onChange={e => setTax(Number(e.target.value))} className="w-32 text-right" />
                </div>
                {editId && (
                  <>
                    <div className="flex items-center justify-end gap-3 mt-4">
                      <Label className="text-emerald-600 font-bold">Amount Paid:</Label>
                      <Input 
                        type="number" min={0} value={paid} 
                        onChange={e => setPaid(Number(e.target.value))}
                        className="w-32 text-right border-emerald-200"
                      />
                    </div>
                    <div className="font-bold text-xl mt-2">Balance Due: {cur} {(total - discount + tax - paid).toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
                  </>
                )}
                {!editId && (
                   <div className="font-bold text-xl pt-2">Net Total: {cur} {(total - discount + tax).toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <Textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Payment terms, notes..." rows={3} />
            </CardContent>
          </Card>
